...
/*
 * engine_em28xx — bridge lifecycle for em28xx-based DVB USB devices.
 *
 ...
 * `engine_em28xx_frontend_t *` instances, which carry the open
...
#define ELOG(...)                                                              \
    do {                                                                       \
        fprintf(stderr, "[engine_em28xx] " __VA_ARGS__);                       \
        fprintf(stderr, "\n");                                                 \
    } while (0)

/* ---- Per-frontend state. Owned by the parent device. ------------- */

typedef struct engine_em28xx_frontend {
    ...
    struct engine_em28xx_dev *dev;

    /* Public handle published to callers. Populated at open. */
    dvb_frontend_handle_t handle;
} engine_em28xx_frontend_t;

typedef struct engine_em28xx_dev {
    usbq_dev_t              *usb;            /* owned */
    em28xx_dev_t            *bridge;         /* owned */
    pthread_mutex_t          bridge_lock;
    const em28xx_board_t    *board;
    engine_em28xx_frontend_t frontends[4];   /* upper bound on fan-out */
    int                      frontend_count;

    struct engine_em28xx_dev *next;          /* registry chain */
} engine_em28xx_dev_t;

static engine_em28xx_dev_t *g_devices = NULL;
static int                  g_usbq_inited = 0;

/* ---- Vtable thunks ----------------------------------------------- *
 *
 * `state` is `engine_em28xx_frontend_t *`. */

static int em28xx_v_tune(void *state, const dvb_tune_params_t *p) {
    engine_em28xx_frontend_t *fe = state;
 ...
 * tears everything down through engine_em28xx_shutdown(). */
...
static int frontend_open(engine_em28xx_dev_t *dev, int fn,
                         struct i2c_adapter *parent_adap) {
    engine_em28xx_frontend_t *fe = &dev->frontends[fn];
    fe->dev          = dev;
    fe->ts_index     = fn;
...
static void frontend_teardown(engine_em28xx_frontend_t *fe) {
    if (!fe) return;
...
static int open_device(const em28xx_board_t *board, const char *vidpid,
    ...
    if (board->num_frontends > (int)(sizeof(((engine_em28xx_dev_t *)0)->frontends)
                                     / sizeof(((engine_em28xx_dev_t *)0)->frontends[0]))) {
    ...
    engine_em28xx_dev_t *dev = calloc(1, sizeof(*dev));
    if (!dev) {
...
void engine_em28xx_shutdown(void) {
    while (g_devices) {
        engine_em28xx_dev_t *dev = g_devices;
        g_devices = dev->next;
