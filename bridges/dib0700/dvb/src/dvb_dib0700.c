...
/*
 * dvb_dib0700 — bridge lifecycle for dib0700-based DVB USB
 * devices. Same shape as dvb_em28xx; the dib0700-specific
...
#define DLOG(...)                                                              \
    do {                                                                       \
        fprintf(stderr, "[dvb_dib0700] " __VA_ARGS__);                      \
        fprintf(stderr, "\n");                                                 \
    } while (0)

typedef struct dvb_dib0700_frontend {
    ...
    struct dvb_dib0700_dev *dev;
    dvb_frontend_handle_t handle;
} dvb_dib0700_frontend_t;

typedef struct dvb_dib0700_dev {
    usbq_dev_t                *usb;
    dib0700_dev_t             *bridge;
    pthread_mutex_t            bridge_lock;
    const dib0700_board_t     *board;
    dvb_dib0700_frontend_t  frontends[2];   /* current ceiling */
    int                        frontend_count;

    struct dvb_dib0700_dev *next;
} dvb_dib0700_dev_t;

static dvb_dib0700_dev_t *g_devices      = NULL;
static int                   g_usbq_inited  = 0;

/* ---- Vtable thunks ----------------------------------------------- */

static int dib_v_tune(void *state, const dvb_tune_params_t *p) {
    dvb_dib0700_frontend_t *fe = state;
...
static int frontend_open(dvb_dib0700_dev_t *dev, int fn,
                         struct i2c_adapter *parent_adap) {
    dvb_dib0700_frontend_t *fe = &dev->frontends[fn];
    fe->dev          = dev;
    fe->fe_index     = fn;
...
static void frontend_teardown(dvb_dib0700_frontend_t *fe) {
    if (!fe) return;
...
static int open_device_from_usb(const dib0700_board_t *board, usbq_dev_t *usb,
    ...
    dvb_dib0700_dev_t *dev = calloc(1, sizeof(*dev));
    if (!dev) {
        usbq_release_interface(usb, 0);
        usbq_close(usb);
        return 0;
...
int dvb_dib0700_open_by_addr(uint8_t bus_number, uint8_t device_address,
    ...
    const dib0700_board_t *board = find_board_by_vidpid(vidpid);
    if (!board) {
        DLOG("no dib0700 board record for %s @ bus %u devaddr %u",
